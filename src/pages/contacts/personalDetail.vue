<template>
  <div class="col-xl-8 xl-50 col-md-7 box-col-6">
      ...
      <div class="tab-pane contact-tab-0 tab-content-child fade " :class="[contact.active ? 'active show' : '']"
        ...
        <div class="profile-mail">
          <div class="media align-items-center"><img class="img-100 img-fluid m-r-20 rounded-circle update_img_0"
              :src="contact.imgUrl || getImgUrl(contact.image)" alt="">
            <input class="updateimg" type="file" name="img" @change="(event) => { loadFile(event, contact) }">
            <div class="media-body mt-0">
              <h5><span class="first_name_0">{{ contact.name1 }} </span> <span class="last_name_0">{{ contact.name2 }}</span>
              </h5>
              ...
              <ul>
                ...
                <li><a href="javascript:void(0)"
                    @click="() => { this.$store.dispatch('contacts/setSelectedContact', contact) }" data-bs-toggle="modal"
          ...
          <div class="email-general">
            ...
            <ul>
              ...
              <li>Birthday<span class="font-primary"> <span class="birth_day_0">{{ contact.day }}</span><span
                    class="birth_month_0 ms-1">{{ contact.month }}</span><span
    ...
    <div class="contact-editform" :style="filter ? '' : 'display: none;'">
      <form>
        <editView />
        <button class="btn btn-secondary  me-2" type="button" @click="collapse()">Save</button>
